...
// - > y(i,j) = x(i,j) – x(i,j+1) //y(i,j) = x(i,j) – x(i+1,j)
...
int main()
{
	...
	for (i = 0; i < HEIGHT; i++) {
		for (j = 0; j < WIDTH; j++) {
			temp_1 = img_in1[i][j] - img_in1[i][j + 1];            //horizontal 엣지 검출 y = f(x,y) - f(x+1,y) 연산수행, 엣지검출시 사용하는 수식, 1차미분
			temp_2 = img_in1[i][j] - img_in1[i + 1][j];            //vertival   엣지 검출 y= f(x,y) - f(x,y+1) 연산수행

			temp_3 = img_in2[i][j] - img_in2[i][j + 1];            //f(x,y) - f(x+1,y) 연산수행
			temp_4 = img_in2[i][j] - img_in2[i + 1][j];            //f(x,y) - f(x,y+1) 연산수행

			img_out_1[i][j] = (BYTE)floor((((temp_1 + 255) / 2.0) + 45)); //for dynamic range // +45는 이미지를 잘 보이게 하기위해 더해준(shift) 임의의 값.
